    ...
    <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
      {items.map((item) => (
        <div key={item.label} className="border-t-2 border-[var(--text-primary)] pt-4">
          <div className="text-[32px] md:text-[40px] font-extrabold tabular-nums tracking-tight">{item.value}</div>
          <div className="mt-1 font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--text-secondary)] font-bold">
            {item.label}
          </div>
        </div>
      ))}
